...
description: Arquitetura de aplicações 3D em React — separação em camadas entre cena R3F e UI, gerência de estado compartilhado entre render 3D e componentes, ciclo de vida e limpeza de recursos, e performance quando várias camadas disputam o mesmo frame. Use ao estruturar a fronteira cena/UI, decidir onde o estado da cena deve viver, diagnosticar re-render que derruba FPS, ou integrar bibliotecas de animação (GSAP ScrollTrigger, Motion, React Spring) com Three.js.
metadata:
  source: freshtechbro/claudedesignskills
  note: as seções de GSAP/Motion/React Spring são referência — nenhuma dessas
    bibliotecas é dependência deste repositório hoje.
---
...
## Skills relacionadas neste repositório

- **`webar-frame-viewer`** — arquitetura concreta do pacote deste repo: engines
  WebXR vs passthrough, hit-test, fronteira `core/` vs `react/`. É a referência
  para qualquer alteração em `packages/webar-frame-viewer`.
- **`3d-web-experience`** — a cena em si: materiais, luzes, câmera, loaders,
  instancing, fallback sem WebGL.
- **`vercel-react-best-practices`** — regras de performance React aplicáveis à
  camada de UI que envolve a cena (memo, estado derivado, re-render).

## Aplicabilidade a este projeto

Este repositório usa **R3F 9 + `@react-three/xr` 6 + three 0.185**, sem GSAP,
Motion ou React Spring. Portanto:

- **Aplicam-se diretamente**: separação em camadas, estado compartilhado entre
  cena e UI, ciclo de vida/limpeza de recursos, armadilhas de performance,
  matriz de decisão.
- **São referência para o futuro**: tudo que depende de GSAP ScrollTrigger,
  Motion ou React Spring. Não introduza nenhuma dessas dependências sem
  discutir antes — o pacote é publicado com bundle sob orçamento (`npm run
  size`) e cada uma delas pesa.
